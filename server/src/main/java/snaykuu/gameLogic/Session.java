...
public class Session implements Game
{
	...
	public snaykuu.gameLogic.Board getBoard()
	{
		return board;
	}
	...
	private boolean perhapsSpawnFruit()
	{
		...
		while (!spawned)
		{
			...
			{
				board.addGameObject(Fruit.INSTANCE, potentialFruitPosition);
				spawned = true;
			}
		}
		return true;
	 ...
	 */
	private snaykuu.gameLogic.Board createStandardBoard(int width, int height)
	{
		...
		for (int x = 0; x < width; x++)
		{
			...
			board.addGameObject(Wall.INSTANCE, bottomRowPos);
			board.addGameObject(Wall.INSTANCE, topRowPos);
		}
		for (int y = 0; y < height; y++)
		{
			Position leftmostColumnPos = new Position(0, y);
			Position rightmostColumnPos = new Position(width-1, y);
			board.addGameObject(Wall.INSTANCE, leftmostColumnPos);
			board.addGameObject(Wall.INSTANCE, rightmostColumnPos);
		}
		return board;
	}
